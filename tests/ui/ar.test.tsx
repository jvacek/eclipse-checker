...
describe('ARView (8th Wall engine)', () => {
  ...
  it('holds the captured yaw offset: later heading events do not move the sun', async () => {
    const { orientation, sky } = renderActive();
    const section = document.querySelector('.ar-view');
    await waitFor(() => expect(section).toHaveAttribute('data-status', 'active'));
    ...
    // The engine world frame is fixed for the session, so the yaw offset is a
    // constant captured once per fix. A later (re-baselined) reading must not
    // yank the sun away from the world-anchored ring — that is what "recalibrate"
    // is for.
    ...
    expect(sun.position.x).toBeCloseTo(Math.sin(az) * Math.cos(alt) * SUN_DISTANCE, 6);
    expect(sun.position.z).toBeCloseTo(-Math.cos(az) * Math.cos(alt) * SUN_DISTANCE, 6);
  });
